/**************************************************************************
 *
 *  File: slow_vector.cpp
 *
 *  Purpose: Implement vector utlizing constant increment strategy for reallocation.
 *
 *  Author:
 *
 *  Date:
...
// Implement the methods for the class. Insert should implement the constant
// increment strategy for the new allocation size.
class SlowVector
{
private:
    int capacity;            // Current max size of the array
    int size;                // Current number of elements in the array
    int expansion_increment; // The increment in capacity in case of an overflow. Recommended value is 10; Initialize it in the constructor.
    int *arr;
    int start = 0;
    int end = 0;
...
public:
    ...
    SlowVector();          // Default constructor initializing all the fields; Please initialize capacity to 1.
...
/* 0 <= i <= Size() */
void SlowVector::Insert(int i, int elem)
{
    if (size < capacity)
    {
        if (i == 0)
    ...
    {
        int *new_arr = new int[capacity + 10];
...
/* 0 <= i <= Size()-1 */
void SlowVector::Set(int i, int elem)
{
    int posn = start;
    posn = add1(posn, i);
    arr[posn] = elem;
